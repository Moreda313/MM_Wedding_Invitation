...
for (const day of onlyDay ? [Number(onlyDay)] : [1, 2]) {
  // A changed Day 2 illustration gets new URLs, including in browser caches.
  const revision = day === 2 ? `-${createHash("sha256").update(await readFile(`day${day}_place.png`)).digest("hex").slice(0, 8)}` : "";
