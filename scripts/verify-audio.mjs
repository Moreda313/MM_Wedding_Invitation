...
try {
  ...
  await page.route('**/audio/flower-dance-loop-v2.mp3', route => route.fulfill({status:404,body:''}));
  ...
  await page.unroute('**/audio/flower-dance-loop-v2.mp3');
  await page.locator('.music-control').click();
  await page.waitForFunction(() => document.querySelector('.invitation').dataset.audioTrack === 'day2');
  await page.route('**/audio/fall-loop-v2.mp3', async route => { await new Promise(resolve => setTimeout(resolve, 1200)); await route.continue(); });
