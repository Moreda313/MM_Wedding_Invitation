...
src/
  ...
  sections/DayTwo.tsx           第二天主图、日程和可轻触互动的活动地图
...
活动地图沿用原布局：咖啡木色、冰淇淋与花束浆果色、钓鱼湖蓝、网球苔绿、种子金盏黄等，统一米白标签，通过像素边线区分。点击活动图标会短促弹动，不再勾选或保存选择。五棵秋树可轻触摇动，每次飘落四片像素叶子；连续点击只重启当前动效，不累积粒子。减少动态效果模式改为轻微透明度反馈，不摇动、不落叶；也支持键盘 Enter / Space。转场采用白绿 → golden hour → 秋日米杏的纵向渐变；后半段顺序为活动地图 → 种子寄语 → 秋日照片墙 → 夕阳告别 → 婚礼信息汇总。告别页对像素新人做 CSS 局部取景，原图不变。
...
手机回滑时浏览器工具栏可能改变可视高度。`useStoryViewport` 在首次布局时测量 `100svh`（旧浏览器回退 `100vh`），手机仅在宽度变化、如旋转屏幕时重新测量，避免高度变化重排开场、婚纱照、转场及后续内容；桌面窗口仍随尺寸变化适配。所有整屏布局共用 `--story-vh`，滚动动画按实际 sticky 容器高度计算进度，音乐与开场 UI 也使用相同的稳定高度。不强制修改滚动位置，不禁止向上返回。`verify-reverse-scroll.mjs` 在 Chrome / WebKit 模拟工具栏高度变化，覆盖开场临界点、正反滚动、照片墙位置、导航浮现、横竖屏切换、桌面缩放、直接锚点和减少动态效果；模拟结果不替代微信真机验证。
...
活动区文案为“在一起，享受秋日时光。”。地图用秋季枫树、橡树、南瓜、蔓越莓、榛子、苹果、葡萄、鸡油菌、玫瑰仙子、小鸡和祝尼魔组合成小景，配合 CSS 木桥、篱笆、草丛和落叶；秋树为独立互动按钮，其他装饰不接收点击，音乐摊位使用长笛块（Flute Block）。种子寄语为“带一份种子回家，让美好慢慢生长。”，搭配混合种子与星之果实（Stardrop）。秋日新增图片合计约 10 KB，执行 `node scripts/prepare-autumn.mjs` 可重建；秋树仅裁出原季节图中的秋季列，保留透明背景与原始像素。
...
PREVIEW_URL=http://127.0.0.1:5173/ node scripts/verify-reverse-scroll.mjs
```
