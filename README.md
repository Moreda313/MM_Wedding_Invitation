...
- 两处高德导航链接与复制地址按钮。
- Day 1：上午接新娘，下午在雷迪森酒店草坪举行婚礼，晚上晚宴。Day 2：11:00 华侨东方大酒店午宴，14:00 遂昌源口大草坪婚礼与派对。正文与汇总页共用日程数据；遂昌地图按钮明确为“午宴酒店地图”，草坪导航待补充，不复用酒店定位。
...
“我们要结婚啦”一屏加入 🎉 和星露谷手捧花。Day 2 转场直接用 “One more thing…” 引导，像素化时展示“人生，也可以是星露谷。”，保留“把日子，过成喜欢的游戏。”作为副文案，也可直接点击“进入第二天”像素按钮。
...
两处场地已接入用户生成的 `day1_place.png` 与 `day2_place.png`。场地图紧跟标题，图下注明“婚礼场地”和具体草坪名，然后呈现日期、酒店和日程；移除信息卡的“现场音乐”字段。执行 `node scripts/prepare-venues.mjs` 生成 640、960、1440 宽的 WebP（质量 88），完整保留 4:3 构图，手机通过 srcset 按需加载，另有“查看大图”入口。各档约 124–143 KB、260–282 KB、497–505 KB，原始 PNG 不覆盖且不提交到仓库。配置支持 src、srcSet、full、width、height；有实际场地照片后可替换。`pixel.bride`、`pixel.groom`、`pixel.couple` 继续预留定制像素人物。
...
活动地图新增秋季枫树、橡树、南瓜、蔓越莓、榛子和散落的小叶子，装饰层不接收点击。删除活动区的标记/打卡说明；种子寄语为“带一份种子回家，让美好慢慢生长。”，搭配混合种子与星之果实（Stardrop）。新图共约 9 KB，执行 `node scripts/prepare-autumn.mjs` 可重建；秋树仅裁出原季节图中的秋季列，保留透明背景与原始像素。
...
- 补充第一天接亲、下午仪式和晚宴的具体钟点，以及遂昌源口大草坪的导航链接。
